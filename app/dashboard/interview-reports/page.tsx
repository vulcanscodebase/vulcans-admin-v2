...
export default function InterviewReportsPage() {
  ...
  const [podStatistics, setPodStatistics] = useState<PodStatistics[]>([]);
  ...
  const loadInterviews = async () => {
    try {
      ...
      const res = await getAllInterviewReports(params);
  ...
  const loadPodStatistics = async () => {
    try {
      setStatsLoading(true);
      const res = await getPodInterviewStatistics();
      setPodStatistics(res.data?.podStatistics || []);
    } catch (error: any) {
      toast.error(
        error.response?.data?.message ||
          "Failed to load pod statistics"
      );
    } finally {
        ...
        <div className="px-4 sm:px-6 lg:px-8 py-8">
          ...
          <div className="mb-6">
            ...
            <p className="text-muted-foreground">
              View and analyze all user interview reports across pods
            </p>
          </div>
          ...
          <Card className="mb-6">
            <CardHeader>
              <CardTitle className="flex items-center">
                <BarChart3 className="mr-2 h-5 w-5 text-vulcan-accent-blue" />
                Pod-wise Statistics
              </CardTitle>
              <CardDescription>
                Interview statistics organized by pod
              </CardDescription>
            </CardHeader>
            <CardContent>
              {statsLoading ? (
              ...
              ) : podStatistics.length === 0 ? (
                <div className="text-center py-8 text-muted-foreground">
                  No pod statistics available
                </div>
              ) : (
                <div className="overflow-x-auto">
                  <table className="min-w-full divide-y divide-gray-200">
                    ...
                    <tbody className="bg-white divide-y divide-gray-200">
                      {podStatistics.map((podStat) => (
                        <tr key={podStat.pod._id}>
                          <td className="px-6 py-4 whitespace-nowrap">
                            <div className="text-sm font-medium text-gray-900">
                              {podStat.pod.name}
                            </div>
                            <div className="text-sm text-gray-500">
                              {podStat.pod.institutionName ||
                                podStat.pod.organizationName ||
                                podStat.pod.type}
                            </div>
                          </td>
                          <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-500">
                            {podStat.statistics.totalUsers}
                          </td>
                          <td className="px-6 py-4 whitespace-nowrap text-sm font-medium text-gray-900">
                            {podStat.statistics.totalInterviews}
                          </td>
                          <td className="px-6 py-4 whitespace-nowrap text-sm text-green-600">
                            {podStat.statistics.completed}
                          </td>
                          <td className="px-6 py-4 whitespace-nowrap text-sm text-yellow-600">
                            {podStat.statistics.inProgress}
                          </td>
                          <td className="px-6 py-4 whitespace-nowrap text-sm text-red-600">
                            {podStat.statistics.abandoned}
                          </td>
                          <td className="px-6 py-4 whitespace-nowrap text-sm">
                            <Button
                              ...
                              onClick={() =>
                                router.push(
                                  `/dashboard/interview-reports/pod/${podStat.pod._id}`
                                )
                              }
                            >
